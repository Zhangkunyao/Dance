...
def define_C(input_nc, ndf, class_num, norm='batch', init_type='normal', gpu_ids=[],witch_D = 'resnet18'):

    norm_layer = get_norm_layer(norm_type=norm)
    # if witch_D == 'resnet18':
    netD = ClassDiscriminator_Resnet(input_nc, class_num, ndf)
    # else:
...
class ClassDiscriminator_Resnet(nn.Module):
    def __init__(self, input_nc, out_nc,pretrain = False):
    ...
    def define_module(self, model):
        self.basic = []
        self.basic += model.conv1
        # self.bn1 = model.bn1
        self.basic += model.relu
        self.basic += model.maxpool
        self.basic += model.layer1
        self.basic += model.layer2
        self.basic += model.layer3
        self.basic += model.maxpool
        self.basic += model.layer4 # 7*7*512
        self.basic = nn.Sequential(*self.basic)

        seq_classfiy = [nn.Conv2d(512,128,kernel_size=(3, 3), stride=(1, 1), bias=True),
                        nn.LeakyReLU(0.2, True)]
        seq_classfiy += [nn.Conv2d(128, self.out_dim, kernel_size=(1, 1), stride=(1, 1), bias=True)]
        self.convlast = nn.Sequential(*seq_classfiy)
        # 接layer3
        seq = [nn.Conv2d(512, 128, kernel_size=(3, 3), stride=(1, 1),padding=(1, 1), bias=False),
            nn.LeakyReLU(0.2, True)]
        seq += [nn.Conv2d(128, 1, kernel_size=(1, 1), stride=(1, 1), bias=True)]
        self.convtf = nn.Sequential(*seq)
    ...
    def forward(self, x):
        ...
        x = self.basic(x)
        tf_out = self.convtf(x)
        tf_out = F.sigmoid(tf_out)

        label_out = self.convlast(x)
        label_out = self.global_average_pooling(label_out)
