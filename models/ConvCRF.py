...
class ConvCRF_Cell():
    def __init__(self,opt):
        if not hasattr(torch.nn.functional, 'unfold'):
            ...
            self.pyinn = False
        shape = (opt.fineSize,opt.fineSize)
        config = default_conf
        config['filter_size'] = 17
        config['col_feats']['schan'] = 0.1
        print("Build ConvCRF.")
        self.gausscrf = GaussCRF(conf=config, shape=shape, nclasses=2).cuda()

    def forward(self,image,label):
        unary = synthetic.augment_label(label, num_classes=2)  # 出来onehot编码的sal_map
        unary_var = Variable(torch.Tensor(unary)).cuda()
        # Perform CRF inference
        prediction = self.gausscrf.forward(unary=unary_var, img=image)
